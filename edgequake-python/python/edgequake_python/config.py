...
@dataclass
class EdgeQuakeConfig:
    """Global configuration for edgequake-python.
    ...
    default_provider: str = field(
        default_factory=lambda: os.environ.get("EDGEQUAKE_PROVIDER", "openai")
    )
    default_model: str = field(
        default_factory=lambda: os.environ.get("EDGEQUAKE_MODEL", "gpt-4o-mini")
    )
    timeout: float = field(
        default_factory=lambda: float(os.environ.get("EDGEQUAKE_TIMEOUT", "60"))
    )
    max_retries: int = field(
        default_factory=lambda: int(os.environ.get("EDGEQUAKE_MAX_RETRIES", "3"))
    )
    drop_params: bool = True
    verbose: bool = field(
        default_factory=lambda: os.environ.get("EDGEQUAKE_VERBOSE", "").lower() in ("1", "true", "yes")
    )


# Singleton config — mutate at startup just like litellm globals
_config = EdgeQuakeConfig()


def get_config() -> EdgeQuakeConfig:
...
# ---------------------------------------------------------------------------
# CompletionOptions builder — serialises to the JSON consumed by _eq_core
# ---------------------------------------------------------------------------
